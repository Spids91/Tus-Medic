// ─── DETAIL.JS ────────────────────────────────────────────────────────────────
// Drug detail overlay: open, close, build HTML
...
function openDet(id) {
  ...
  document.getElementById('detBadges').innerHTML =
    ...
    `<span class="det-mbadge" style="background:${MASTERY_COLORS[m]}22;color:${MASTERY_COLORS[m]}">
      ${MASTERY_LABELS[m]} (${correct}/10)
    </span>`;
...
function buildDet(d) {
  ...
  return `
    <div class="dsec">
      <div class="dsh"><div class="sico" style="background:var(--em-50)">📈</div><div class="dst">Mastery Progress</div></div>
      <div class="dsb">
        <div class="prog-wrap"><div class="prog-fill" style="width:${pct}%;background:${MASTERY_COLORS[m]}"></div></div>
        <div class="prog-lbl"><span>${MASTERY_LABELS[m]}</span><span>${correct}/10 correct</span></div>
      </div>
    </div>
    ...
    <div class="dsec">
      ...
      <div class="dsb">
        <div class="dose-block">
          <div class="dose-grp"><div class="dose-lbl">👤 Adult</div>${doseHtml(d.dosages.adult)}</div>
          <div class="dose-grp"><div class="dose-lbl">👶 Paediatric</div>${doseHtml(d.dosages.paediatric)}</div>
        </div>
      </div>
    </div>
    ...
    <div class="dsec">
      ...
      <div class="dsb">
        <textarea
          ...
          oninput="G.notes[${d.id}]=this.value;saveG()"
        >${noteVal}</textarea>
